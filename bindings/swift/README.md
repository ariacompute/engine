...
XCFramework / module map over `libariaengine_ffi`. CocoaPods name: `AriaEngine`.
`AriaEngine.open` installs `libariaengine_ffi` into `~/.ariacompute/lib/` when missing (same Releases asset as `ariaengine upgrade`). `ARIAENGINE_FFI_LIB` remains an optional override.
...
(same as `ariaengine download`: `.com` → Hugging Face, `.cn` → ModelScope;
...
`hfToken` / `modelscopeApiToken` (same as `ariaengine setup`); if omitted, reads
