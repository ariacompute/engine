...
| 5 | **router 接入** | `engine.yml`：仅 `router` + `router_api_key`（`sk-aria_` **或** `sk-bf-`，用户无区分）；aria-router 按前缀关联本地 keys 或 oauth。`--router` / `--router-api-key` 覆盖本进程。**禁止** `cloud_handoff` / `route_hybrid`。 |
...
| `router_api_key` | 可选；`sk-aria_…` 或 `sk-bf-…`（router 按前缀关联）；注册时 Bearer |
...
- `setup`：交互询问 hub / compute / 可选 router URL 与 **router API key**（`sk-aria_` 或 `sk-bf-`）；`--status` 脱敏；`--clear` 删除 `engine.yml`。无 `auth` 子命令。
...
**按模型名下载：** 与 `aria-engine download` 相同，仅本区公开 hub（`.com`→Hugging Face，`.cn`→ModelScope）。**不**请求 Dashboard zip meta。Hub 凭证字段与 `aria-engine setup` 相同：`hf_token`（`.com`）/ `modelscope_api_token`（`.cn`）。调用时可显式传入（含 `Engine.setup` 实例内存）；未传则读 `~/.ariacompute/engine.yml`（缺则回退旧 `config.yml`）。**不**读环境变量 `HF_TOKEN` / `MODELSCOPE_API_TOKEN`。Dashboard `sk-` token 不作为 hub Bearer。公开模型无需 token。缓存 `~/.ariacompute/models/{model}`；已有有效 bundle 则跳过下载。
