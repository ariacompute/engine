...
| P0 | **Qwen3.5 / Bonsai** | linear_attention / Gated DeltaNet 层：实现或对未实现层返回 `Unsupported`；禁止当全 SDPA dense。消费 HF Qwen3.5 拆分投影（`in_proj_qkv`+`in_proj_z`、`in_proj_b`+`in_proj_a`），兼容 Qwen3-Next 融合 `in_proj_qkvz` / `in_proj_ba` |
