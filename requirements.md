...
| P1 | **Gemma 正确性** | GeGLU（`gelu_pytorch_tanh`）vs SwiGLU；RMSNorm `*(1+w)`；四 norm；`ffn_norm` 优先 `pre_feedforward_layernorm`；Gemma-4：**KV cache 按注意力类型复用**（非 clone `wk`/`wv`）、滑动窗口 mask（默认 512）、双/部分 RoPE、PLE |
